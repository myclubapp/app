...
export class EventService {
  ...
  getClubEventsRef(clubId: string): Observable<Event> {
    ...
    return collectionData(eventsRefList, { idField: 'id' }) as unknown as Observable<Event>;
  }


  /* TEAM EventS ATTENDEES */
  getTeamEventsAttendeesRef(teamId: string,eventId: string): Observable<any[]> {
    // console.log(`Read Team Events Attendees List Ref ${teamId} with Event ${eventId}`)
    const attendeesRefList = collection(this.firestore, `teams/${teamId}/events/${eventId}/attendees`);
    return collectionData(attendeesRefList, { idField: 'id' }) as unknown as Observable<any[]>;
  }

/* TEAM EventS ATTENDEE Status */
  setTeamEventAttendeeStatus(userId: string, status: boolean, teamId: string,eventId: string) {
    const statusRef = doc(this.firestore,`teams/${teamId}/events/${eventId}/attendees/${userId}`);
