...
export class NewsService {
  ...
  async createNews(
    ...
    this.newsListRef = firebase.default.firestore().collection(`userProfile/${user.uid}/newsList`);
  ...
  async deleteTeamNews(news) {
    return firebase.default.firestore().collection(`team`).doc(`${news.teamId}`).collection(`newsList`).doc(`${news.id}`).delete();
  }

  async changeTeamNews(news) {
    return firebase.default.firestore().collection(`team`).doc(`${news.teamId}`).collection(`newsList`).doc(`${news.id}`).set(news, {merge: true});
  }
  ...
  async deleteClubNews(news) {
    return firebase.default.firestore().collection(`team`).doc(`${news.clubId}`).collection(`newsList`).doc(`${news.id}`).delete();
  }

  async changeClubNews(news) {
    return firebase.default.firestore().collection(`club`).doc(`${news.clubId}`).collection(`newsList`).doc(`${news.id}`).set(news, {merge: true});
  }
}
