...
export class ChampionshipService {
  ...
  async acceptGame(game){
    ...
    .firestore()
    .collection('team').doc(String(game.teamId)).collection('championshipList').doc(`${String(game.id)}`).collection('memberList').doc(`${user.uid}`).set({
      status: true
    });
  ...
  async getGameStatus(teamId:String, gameId:String): Promise <firebase.default.firestore.DocumentSnapshot>{
   ...
   // console.log(teamId + "/" + gameId);
    ...
    .firestore()
    .collection('team').doc(String(teamId)).collection('championshipList').doc(`${String(gameId)}`).collection('memberList').doc(`${user.uid}`).get();
  }
  ...
  async getAcceptList(game): Promise < any > {
    ...
    .firestore()
    .collection('team').doc(String(game.teamId)).collection('championshipList').doc(`${game.id}`).collection("memberList").where("status", "==", true);
  ...
  async getRejectList(game): Promise < any > {
    ...
    let statusList = firebase.default
    .firestore()
    .collection('team').doc(String(game.teamId)).collection('championshipList').doc(`${game.id}`).collection("memberList").where("status", "==", false);
    let list = await  statusList.get();
    //console.log(list);
    for (let element of list.docs){
