...
export class HelferPunkteDetailPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.helferPunkteList$ = this.getHeferEinsatz(
      this.helferData.profile.id,
      this.clubId,
    );
    this.clubMember$ = this.fbService.getClubMemberRef(
      this.clubId,
      this.helferData.profile.id,
    );
  ...
  updateHelferPunkte(helferPunkte: any) {
    ...
    this.fbService.setHelferPunkte(
      this.clubId,
      this.helferData.profile.id,
      Number(helferPunkte),
    );
  ...
  getHeferEinsatz(profileId: string, clubId: string) {
    ...
    return this.helferService.getUserHelferPunkteRefs(profileId, clubId).pipe(
      ...
      tap((helferPunkte) => {
        ...
        const years = new Set(
          helferPunkte.map((punkt) => punkt.eventDate.toDate().getFullYear()),
        );
    ...
    const alert = await this.alertController.create({
      ...
      inputs: [
        ...
        },
        {
          ...
          type: "date" as const,
          label: "Datum",
          value: helferPunktEvent.eventDate
            ?.toDate()
            .toISOString()
            .split("T")[0],
        },
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async (data) => {
            ...
            try {
              await this.helferService.updateHelferPunkt(
                this.clubId,
                helferPunktEvent.id,
                {
                  name: data.name,
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            try {
              await this.helferService.deleteHelferPunkt(
                this.clubId,
                helferPunktEvent.id,
              );
