...
export class HelferAddPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription = this.clubId$.subscribe((value) => {
      ...
      this.clubGamesList$ = this.championshipService
        .getClubGamesRef(value)
        .pipe(take(1));
    });
  }
  ...
  async createEvent() {
    ...
    if (confirmed) {
      ...
      calculatedStartDate.setMinutes(
        new Date(this.event.timeFrom).getMinutes(),
      );
      ...
      const helferEvent = await this.eventService
        .setCreateHelferEvent(this.event)
        .catch((e) => {
