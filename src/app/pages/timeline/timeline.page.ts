...
import {
  TeamListPage
} from './../team-list/team-list.page';
import {
  ClubListPage
} from './../club-list/club-list.page';
...
export class TimelinePage implements OnInit {
  ...
  constructor(
    ...
    this.profileService.getUserProfileChanges().then(user=>{
...
clubListRef.onSnapshot(async snapshot => {
...
*/
    ...
    this.teamSerivice.getClubList().then(clubList => {
      ...
      for (let club of clubList) {
        ...
        this.requestService.getClubRequestList(club.id).onSnapshot(async querySnapshot => {
  ...
  approveRequest(id) {
    ...
    this.requestService.approve(id).then(ok=>{
  ...
  rejectRequest(id) {
    this.requestService.reject(id).then(ok=>{
      this.toastController.create({
