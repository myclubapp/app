...
export class NewsCreatePage implements OnInit {
  ...
  createNews(): void {
    if (this.news.title === undefined || this.news.description === undefined) {
    ...
    this.newsService.createNews(this.news.title, this.news.description, this.news.clubId, this.news.teamId).then(
      () => {
        this.toastController
          .create({
